...
const char* ConvertLogLevelToString(LogLevel level) {
    switch (level) {
        ...
        case WARN:    return "WARN   "; // CM: Delete when Kyle is ready
//        case WARNING: return "WARNING"; // CM: Add when Kyle is ready
        case ERROR:   return "ERROR  "; // CM: Delete when Kyle is ready
//        case SEVERE:  return "SEVERE "; // CM: Add when Kyle is ready
...
LogLevel ConvertStringToLogLevel(const char* str) {
    ...
    if (strcasecmp(trimmedStr, "WARN")    == 0) return WARN;  // CM: Delete when Kyle is ready
//    if (strcasecmp(trimmedStr, "WARNING") == 0) return WARNING;  // CM: Change when Kyle is ready
    if (strcasecmp(trimmedStr, "ERROR")   == 0) return ERROR; // CM: Delete when Kyle is ready
//    if (strcasecmp(trimmedStr, "SEVERE")  == 0) return SEVERE; // CM: Change when Kyle is ready
...
void WriteLogTestMsgs() {
    ...
    setenv(EV_MODULE_LOGLEVEL, "ERROR", 1);   Log(ERROR, "Sample Log for LogLevel::ERROR");
    setenv(EV_MODULE_LOGLEVEL, "WARN", 1);    Log(WARN, "Sample Log for LogLevel::WARN");
